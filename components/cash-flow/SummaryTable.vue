<template>
    <table class="w-full text-left rtl:text-right border-collapse lg:min-w-full min-w-[1100px]">
        ...
        <tbody>
            ...
            <!-- Incoming (expandable) -->
            <tr class="transition-all duration-500" :class="isDark ? 'bg-transparent border-b border-white/10' : 'bg-white border-b border-gray-100'">
                <td :class="[cellPad, isDark ? 'text-white' : 'text-black', 'font-medium text-[14px]']">
                    <div class="flex items-center gap-2 cursor-pointer" @click="showIncoming = !showIncoming">
            ...
            <template v-if="showIncoming">
                <template v-for="section in (sections?.incoming ?? [])" :key="section.key">
                    ...
                    <tr v-for="row in section.rows" :key="section.key + row.name"
                        :class="isDark ? 'bg-[#04C18F1A] border-b border-white/10' : 'bg-[#E8F9F5] border-b border-gray-200'">
                        <td class="px-8 py-3 text-[13px] font-medium" :class="isDark ? 'text-white' : 'text-black'">
                            <div class="flex items-center gap-2">
                                <span>{{ row.name }}</span>
                                <button v-if="row.invoiceType" @click="openInvoices(row)" :title="currentLang === 'ar' ? 'عرض تفاصيل الفواتير' : 'View Invoice Details'"
            ...
            <tr class="transition-all duration-500" :class="isDark ? 'bg-transparent border-b border-white/10' : 'bg-white border-b border-gray-100'">
                <td :class="[cellPad, isDark ? 'text-white' : 'text-black', 'font-medium text-[14px]']">
                    <div class="flex items-center gap-2 cursor-pointer" @click="showOutgoing = !showOutgoing">
            ...
            <!-- Outgoing sub-sections -->
            <template v-if="showOutgoing">
