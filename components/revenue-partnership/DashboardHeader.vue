<template>
  <header class="h-[110px] w-full flex items-center justify-between px-8 bg-transparent relative z-20">
    ...
    <div class="absolute inset-y-0 right-0 w-full max-w-[90%] pointer-events-none z-[-1] float-right">
      <img :src="isDark ? '/images/partnership-dark-bg.png' : '/images/partnership-bg.png'" alt="Header Background" class="w-full h-full object-fill" />
    </div>
    ...
    <div class="flex items-center gap-6">
      ...
      <div class="flex items-center gap-4 ml-[-8px]">
        ...
        <CommonTooltip :text="currentLang === 'ar' ? 'التنبيهات' : 'Notifications'">
          <NuxtLink to="/revenue-partnership/notifications" class="w-[38px] h-[38px] rounded-full bg-[#00896F]/40 border border-[#ffffff1A] flex items-center justify-center text-white hover:bg-[#00896F]/60 transition-colors cursor-pointer">
        ...
        <CommonTooltip :text="currentLang === 'ar' ? 'تسجيل الخروج' : 'Logout'">
          <NuxtLink to="/revenue-partnership-login" class="w-[38px] h-[38px] rounded-full bg-[#00896F]/40 border border-[#ffffff1A] flex items-center justify-center text-white hover:bg-[#00896F]/60 transition-colors cursor-pointer">
