<template>
    <div class=" ">
        <div class="container mx-auto">
            <div class="flex lg:flex-row flex-col  lg:items-center items-start max-lg:gap-4 justify-between">
                <!-- Title Section -->
                <div>
                    <h1 class="text-2xl font-medium" :class="isDark ? 'text-white' : 'text-[#013E32]'">{{ currentLang
                        === 'ar' ? 'تحليل حسابات الدفع' : 'Accounts Payable Analysis' }}</h1>
                    <p class="text-sm mt-1" :class="isDark ? 'text-white' : 'text-black/59'">{{ currentLang === 'ar' ?
                        'تتبع شامل لحسابات الدفع ورؤى التقادم' : 'Comprehensive AP tracking and aging insights' }}</p>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button
                        class="w-[40px] h-[40px] flex items-center justify-center border rounded-lg transition-colors"
                        :class="isDark ? 'bg-[#002E26] border-[#03D8B0]' : 'bg-white border-primary-100 hover:bg-gray-50'">
                        <img :src="isDark ? '/images/icons/export.svg' : '/images/icons/export.svg'" alt="Export Icon"
                            class="w-5 h-5" :class="isDark ? 'invert' : ''" />
                    </button>
                </div>
            </div>
