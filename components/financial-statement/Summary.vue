<template>
    <!-- Use overflow-x-auto so the table can be scrolled horizontally if it gets too narrow -->
    <div class="w-full overflow-x-auto no-scrollbar transition-all duration-500">
        <div class="pb-8 px-8 flex justify-between items-center">
            <p class="text-base font-medium text-primary-450">Profit & Loss Summary</p>
        ...
        <table class="w-full text-left border-collapse min-w-150">
            <thead class="bg-[#006b54] text-white px-8">
                <tr class="transition-all duration-500">
                    <!-- Dynamic padding based on isCompressed -->
            ...
            <tbody>
                <tr v-for="(row, i) in data" :key="i" :class="[
                    ...
                    <td class="text-center">
                        <span v-if="row.schedule" class="text-[#00a884] underline underline-offset-4 cursor-pointer">{{
                            row.schedule }}</span>
                        <span v-else class="text-gray-300">_</span>
                    </td>
                    ...
                    <td :class="isCompressed ? 'px-2' : 'px-4'" class="text-right">{{ row.previous }}</td>
                    <td :class="isCompressed ? 'px-2' : 'px-4'" class="text-right">{{ row.budget }}</td>
                    <td class="text-center">
                        <span class="px-2 py-1 rounded bg-[#e6f9f4] text-[#00b48d] text-[10px] font-bold">{{
                            row.variance }}</span>
                    </td>
                            ...
                            <div :class="isCompressed ? 'w-7 h-7' : 'w-9 h-9'"
                                ...
                                <svg class="w-full h-full -rotate-90" viewBox="0 0 36 36">
                                    <circle cx="18" cy="18" r="16" fill="none" class="stroke-gray-100"
                                        stroke-width="3" />
                                    <circle cx="18" cy="18" r="16" fill="none" stroke="#00d28e" stroke-width="3"
...
defineProps({
    data: Array,
    isCompressed: Boolean // Receives isChatOpen from parent
})
</script>