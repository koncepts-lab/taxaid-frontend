                ...
                <tr>
                    <template v-if="activeTab === 'ratios'">
                        <template v-if="selectedRatio === 'All Ratios'">
                            <th class="lg:px-8 px-4 py-4 font-medium text-sm text-start">{{ t.colRatioCat }}
                            </th>
                            <th class="font-medium text-center  text-sm px-4">{{ t.colMetric }}</th>
                        </template>
                        <template v-else>
                            <th colspan="2" class="lg:px-8 px-4 py-4 font-medium  text-sm text-start">{{
                                t.colRatio }}
                            </th>
                        </template>
                    </template>
                    <template v-else>
                        <th :class="isCompressed ? `lg:px-8 px-4 py-3` : `lg:px-8 px-4 py-4`"
                    ...
                    <th class="font-medium text-center  text-sm ">{{ t.colYTG }}</th>
                </tr>
            </thead>
            <tbody>
                ...
                <template v-else v-for="(row, i) in visibleRows" :key="i">
                    ...
                    <tr v-else :class="[
                        ...
                        <template v-if="activeTab === 'ratios'">
                            ...
                            <template v-else>
                                <td colspan="2" class="lg:px-8 px-4 py-4 text-start text-sm font-medium"
                                    :class="isDark ? 'text-white/80' : 'text-black/80'">{{ row.label }}</td>
                            </template>
                        </template>
                        <template v-else>
                            <td :class="[isCompressed ? 'lg:px-8 px-4 py-3' : 'lg:px-8 px-4 py-4', row.isSummary || row.isTotal ? 'font-medium' : 'font-normal', isDark ? 'text-white/80' : 'text-black/80']"
...
// Add this alongside your existing getProgressColor function
