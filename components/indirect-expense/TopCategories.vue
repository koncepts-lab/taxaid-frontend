<template>
  <div class="w-full h-full rounded-[30px] p-8 relative flex flex-col overflow-hidden"
    ...
    <!-- Chart Area -->
    <div class="flex-1 flex flex-col lg:flex-row items-center justify-center gap-4 lg:gap-12">
      <!-- Chart Container -->
      <div class="relative w-full lg:w-auto flex items-center justify-center min-h-[300px] lg:min-h-0">
      ...
      <div class="w-full lg:w-[45%] flex items-center justify-center">
        <div class="w-full max-w-[320px] py-6 lg:py-8 px-6 lg:px-8 rounded-[35px] bg-[#1A8065] flex flex-col gap-4 lg:gap-6">
          <div v-for="(label, index) in labels" :key="index" 
            class="flex items-center gap-4">
            <div class="w-[15px] h-[15px] rounded-full flex-shrink-0" :style="{ backgroundColor: colors[index] }"></div>
            <span class="text-white text-[13px] lg:text-[14px] font-normal tracking-[0.5px]">{{ currentLang === 'ar' ? labelsAr[index] : label }} ({{ series[index] }}%)</span>
          </div>
        </div>
      </div>
    </div>
