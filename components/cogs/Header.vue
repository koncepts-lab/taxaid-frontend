    ...
    <div class=" ">
        <div class="container mx-auto">
            <div class="flex items-center justify-between">
                <!-- Title Section -->
                <div>
                    <h1 class="text-2xl font-medium" :class="isDark ? 'text-white' : 'text-[#013E32]'">{{ currentLang === 'ar' ? 'تحليل تكلفة المبيعات' : 'COGS Analysis' }}</h1>
                    <p class="text-sm mt-1" :class="isDark ? 'text-white' : 'text-black/59'">{{ currentLang === 'ar' ? 'تفصيل تكلفة المبيعات حسب الفئات' : 'COGS Breakdown by Categories' }}</p>
                </div>

                <!-- Action Buttons -->
                <div class="flex items-center gap-3">
                    <!-- One Click Summary Button -->
                    <button class="flex items-center space-x-2 rtl:space-x-reverse px-4 h-[40px] rounded-lg border transition-colors"
                        :class="isDark ? 'bg-[#002E26] border-[#03D8B0] text-white hover:bg-[#003d35]' : 'bg-white border-[#03D8B0] text-black hover:bg-teal-50'">
                        <svg class="w-4 h-4" :class="isDark ? 'text-[#03D8B0]' : 'text-teal-600'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                        </svg>
                        <span class="text-sm font-normal">{{ currentLang === 'ar' ? 'ملخص بنقرة واحدة' : 'One Click Summary' }}</span>
                    </button>
