<template>
  <aside
    class="fixed left-0 top-0 bottom-0 z-30 w-[100px] flex flex-col items-center py-10 transition-all duration-300 hidden lg:flex"
    style="background-image: url('/images/side-menu-bg.png'); background-size: 100% 100%; background-repeat: no-repeat;">
    <!-- NAV ITEMS -->
    <nav class="flex-1 w-full flex flex-col items-start justify-center gap-y-[3vh] md:gap-y-[5vh] lg:gap-y-[3vh]">
      <NuxtLink v-for="(item, index) in navItems" :key="index" :to="item.to" class="nav-item group relative side-icons">
        ...
        <!-- TOOLTIP -->
        <div
          class="tooltip absolute left-full ml-0 px-4 py-2 bg-white text-[#009276] text-[16px] font-medium rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 whitespace-nowrap z-50">
          {{ item.label }}
          <!-- Arrow -->
          <div class="absolute right-[100%] top-1/2 -translate-y-1/2 border-8 border-transparent border-r-white"></div>
        </div>
      </NuxtLink>
    </nav>
  </aside>
