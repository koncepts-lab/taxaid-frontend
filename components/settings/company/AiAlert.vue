<template>
  <div class="border rounded-[16px] shadow-sm overflow-hidden" :class="isDark ? 'bg-[#002E26] border-teal-950/40' : 'bg-white border-gray-100'">
    <div class="p-6 pb-2">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div class="flex items-center gap-4">
          ...
          <h2 class="text-[18px] font-normal" :class="isDark ? 'text-white' : 'text-[#101828]'">AI Alert Settings</h2>
        </div>
      ...
      <p class="text-[13px] mt-2 mb-6" :class="isDark ? 'text-white/60' : 'text-[#4A5565]'">
      ...
      <div class="min-h-[420px] max-h-[420px] overflow-y-auto pr-1 mb-6">
        <div v-if="loading" class="space-y-6">
          <div v-for="n in 5" :key="'sk'+n" class="border-b pb-6 last:border-0 last:pb-0" :class="isDark ? 'border-white/10' : 'border-[#F3F4F6]'">
            <div class="h-4 w-1/3 rounded animate-pulse mb-2" :class="isDark ? 'bg-white/10' : 'bg-gray-100'"></div>
            <div class="h-3 w-1/2 rounded animate-pulse" :class="isDark ? 'bg-white/10' : 'bg-gray-100'"></div>
          </div>
        </div>
        ...
        <div v-else class="space-y-6">
          <div v-for="group in groups" :key="group.group_id" class="border-b pb-6 last:border-0 last:pb-0" :class="isDark ? 'border-white/10' : 'border-[#F3F4F6]'">
            <div class="flex items-start justify-between gap-4">
              <div class="min-w-0">
                <h3 class="text-[14px] font-medium leading-tight" :class="isDark ? 'text-white' : 'text-[#101828]'">{{ group.name }}</h3>
              ...
              <div v-if="!personal" class="flex items-center gap-4 shrink-0">
                <label class="flex items-center gap-1.5 text-[12px]" :class="isDark ? 'text-white/60' : 'text-gray-500'">
                  Email
                  <button :disabled="!group.is_enabled" @click="toggleGroup(group, 'email_enabled')"
                    ...
                    :class="group.email_enabled ? 'bg-[#00835D]' : (isDark ? 'bg-white/20' : 'bg-gray-200')">
                ...
                <button @click="toggleGroup(group, 'is_enabled')"
                  ...
                  :class="group.is_enabled ? 'bg-[#00835D]' : (isDark ? 'bg-white/20' : 'bg-gray-200')">
            ...
            <template v-if="showRoles && !personal">
              ...
              <div v-if="group._expanded" class="mt-3 flex flex-wrap gap-2">
                <label v-for="role in assignableRoles" :key="role"
                  ...
                  :class="group.roles[role] ? (isDark ? 'bg-teal-900/50 border-[#00B68D] text-[#7DF5D4]' : 'bg-[#E8FCF2] border-[#00835D] text-[#00835D]') : (isDark ? 'text-white/60 border-white/20' : 'text-gray-500')">
