        ...
        <div class=" data-source-container pt-8 lg:pt-0">
          ...
          <div class="flex items-start gap-4 mb-6">
            ...
            <div>
              <h1 class="font-medium transition-colors duration-300" :class="isDark ? 'text-white' : 'text-[#013E32]'" style="font-size: 24px;">
                {{ currentLang === 'ar' ? 'إدخال البيانات الرئيسية للعميل المحتمل' : 'Lead Master Data Entry' }}
              </h1>
              <p class="font-normal mt-1 transition-colors duration-300" :class="isDark ? 'text-white/60' : 'text-[#00000080]'" style="font-size: 14px;">
                {{ currentLang === 'ar' ? 'أدخل معلومات العميل المحتمل وتفاصيل المشروع' : 'Enter lead information and project details' }}
              </p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 mb-10">
            ...
            <div>
              ...
              <div class="relative">
                <div @click="activeCalendar = activeCalendar === 'expected' ? null : 'expected'"
                  ...
                  <span :class="!form.expected_sign_date ? 'text-[#717182]' : 'text-gray-700'" class="text-sm">
                    {{ form.expected_sign_date ? new Date(form.expected_sign_date).toLocaleDateString('en-GB').replace(/\//g, '-') : 'dd-mm-yyyy' }}
                  </span>
                ...
                <transition name="fade-slide">
                  <div v-if="activeCalendar === 'expected'"
                    class="absolute top-[calc(100%+8px)] left-0 z-[60] bg-white border border-gray-100 rounded-2xl shadow-2xl p-2">
                    <VDatePicker v-model="form.expected_sign_date" @update:model-value="activeCalendar = null" color="teal" borderless />
                  </div>
                </transition>
              </div>
            </div>
          ...
          <div class="mb-10">
            <div class="flex justify-between items-center mb-4">
              <h2 class="text-[18px] font-medium text-[#0A0A0A]">Lead Milestones</h2>
              ...
              <table class="w-full text-sm text-left">
                ...
                <tbody class="border-b border-gray-100">
                  <tr v-for="(milestone, index) in paginatedMilestones" :key="milestone.id || index" class="border-b border-gray-100/50">
                    ...
                    <td class="p-2 w-24">
                      <button @click="removeMilestone(milestone)" class="text-red-400 hover:text-red-600 font-medium transition-colors">Delete</button>
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <table class="w-full text-sm text-left">
                ...
                <tbody class="border-b border-gray-100">
                  <tr v-for="(budget, index) in paginatedBudgets" :key="budget.id || index" class="border-b border-gray-100/50">
                    ...
                    <td class="p-2 w-40">
                      <input type="text" v-model="budget.amount" style="background: rgb(243, 243, 245); border: 1px solid rgba(0, 0, 0, 0.04);" class="w-full px-3 py-2.5 rounded outline-none text-gray-700 text-sm placeholder-[#717182]" />
                    </td>
                    <td class="p-2 w-24">
                      <button @click="removeBudget(budget)" class="text-red-400 hover:text-red-600 font-medium transition-colors">Delete</button>
                    </td>
                  </tr>
                </tbody>
              </table>
