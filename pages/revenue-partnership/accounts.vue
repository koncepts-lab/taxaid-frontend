<template>
  <div class="min-h-screen w-full relative flex flex-col font-sans transition-colors duration-300" :class="isDark ? 'dark-mode-bg text-white' : 'bg-[#f3f4f6] text-[#1a1a1a]'">
      ...
      <div v-if="showAlertBanner" :class="isDark ? 'bg-[#00141080] border-[#F9AF4D80]' : 'bg-[#FEFCE8] border-[#FFF085]'" class="rounded-[16px] p-4 flex items-center justify-between shadow-sm relative pr-12 border">
        <div class="flex items-center gap-4">
          <div class="w-10 h-10 bg-[#FFBB0D] rounded-full flex items-center justify-center flex-shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="text-white w-6 h-6 transition-colors" :class="{ 'hover:text-white': isDark }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </div>
          <div>
            <h4 class="text-[16px] font-semibold" :class="isDark ? 'text-white' : 'text-[#854D0E]'">Expiring cards detected</h4>
            <p class="text-[14px] font-normal" :class="isDark ? 'text-white/70' : 'text-[#854D0E]'">3 customers have cards expiring in under 30 days. Update details to prevent payment issues.</p>
          </div>
        </div>
        <button @click="showAlertBanner = false" class="transition-colors cursor-pointer" :class="isDark ? 'text-white/50 hover:text-white' : 'text-[#854D0E] hover:text-black'">
      ...
      <div v-else class="space-y-12">
        ...
        <section>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <!-- Month End Report -->
            <div :class="isDark ? 'bg-[#00141080] border-white/10' : 'bg-[#F0FDF4] border-[#BEDBFF]'" class="rounded-[24px] p-[24px] border shadow-sm group hover:shadow-md transition-all">
              <div class="flex items-center gap-3 mb-8">
                <img src="/images/icons/Month-End-Report.svg" class="w-7 h-7" :class="{ 'brightness-0 invert': isDark }" />
                <h4 class="text-[16px] font-normal" :class="isDark ? 'text-white' : 'text-[#00614E]'">Month End Report</h4>
              </div>
              <div class="space-y-6">
                <div>
                  <label class="block text-[14px] mb-2" :class="isDark ? 'text-white/70' : 'text-[#0A0A0A]'">Upload CSV File</label>
                  <div @click="$refs.csvInput.click()" 
                    ...
                    <span class="font-medium text-[15px]" :class="isDark ? 'text-white' : 'text-[#000]'">Choose File</span>
            ...
            <div :class="isDark ? 'bg-[#00141080] border-white/10' : 'bg-[#F0FDF4] border-[#BEDBFF]'" class="rounded-[24px] p-[24px] border shadow-sm group hover:shadow-md transition-all">
              <div class="flex items-center gap-3 mb-8">
                <img src="/images/icons/Hosting-Cost-Report.svg" class="w-7 h-7" :class="{ 'brightness-0 invert': isDark }" />
                <h4 class="text-[16px] font-normal" :class="isDark ? 'text-white' : 'text-[#00614E]'">Hosting Cost Report</h4>
              </div>
              <div class="space-y-6">
                <div>
                  <label class="block text-[14px] mb-2" :class="isDark ? 'text-white/70' : 'text-[#0A0A0A]'">Upload Excel File</label>
        ...
        <section class="space-y-8">
          ...
          <!-- Sub Tabs -->
          <div :class="isDark ? 'bg-[#00141080] border-white/10' : 'bg-white border-gray-100'" class="rounded-full p-1.5 flex items-center shadow-sm border overflow-x-auto no-scrollbar">
        ...
        <div class="flex items-center gap-4 rounded-[16px] p-5 border transition-all" 
          ...
          <div class="relative w-full max-w-[30%]">
            <svg xmlns="http://www.w3.org/2000/svg" class="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 pointer-events-none" :class="isDark ? 'text-white/60' : 'text-[#00000040]'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div @click="isStatusDropdownOpen = !isStatusDropdownOpen"
              ...
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-[#00000040] transition-transform duration-200" :class="{ 'rotate-180': isStatusDropdownOpen, 'brightness-0 invert': isDark }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <transition name="fade-slide">
              <div v-if="isStatusDropdownOpen" :class="isDark ? 'bg-[#111] border-white/10' : 'bg-white border-gray-100'" class="absolute top-[calc(100%+4px)] left-0 w-full border rounded-[12px] shadow-xl z-50 p-2 space-y-1">
          ...
          <!-- Export Button -->
          <button class="h-[44px] w-[350px] flex items-center justify-center gap-2 px-5 rounded-[8px] text-[14px] font-medium border transition-all cursor-pointer hover:shadow-sm" 
        ...
        <section :class="isDark ? 'bg-[#00141080] border-white/10' : 'bg-white border-gray-100'" class="p-8 rounded-[24px] border shadow-sm overflow-hidden">
          ...
          <div class="overflow-x-auto">
            ...
            <table v-if="activeCustomerSubTab === 'Partners'" class="w-full text-left border-collapse min-w-[1200px]">
              ...
              <tbody class="divide-y divide-gray-50 bg-[#F9FAFB]/50">
                <tr v-for="(row, idx) in paymentDetails" :key="idx" :class="isDark ? 'hover:bg-white/5' : 'hover:bg-white'" class="transition-all group">
                  <td class="px-8 py-6 text-[14px]" :class="isDark ? 'text-white/90' : 'text-[#0A0A0A]'">{{ row.code }}</td>
                  <td class="px-8 py-6 text-[14px]" :class="isDark ? 'text-white/90' : 'text-[#0A0A0A]'">{{ row.source }}</td>
                  <td class="px-8 py-6 text-[14px] text-center" :class="isDark ? 'text-white/90' : 'text-[#0A0A0A]'">{{ row.company }}</td>
                  <td class="px-8 py-6 text-[14px] text-center" :class="isDark ? 'text-white/90' : 'text-[#0A0A0A]'">{{ row.period }}</td>
                  <td class="px-8 py-6 text-[14px] text-center" :class="isDark ? 'text-white/90' : 'text-[#0A0A0A]'">{{ row.year }}</td>
                  <td class="px-8 py-6 text-[14px] text-center" :class="isDark ? 'text-white/90' : 'text-[#0A0A0A]'">{{ row.revenue }}</td>
                  <td class="px-8 py-6 text-[14px] text-center" :class="isDark ? 'text-white/90' : 'text-[#0A0A0A]'">{{ row.collected }}</td>
                  <td class="px-8 py-6 text-[14px] text-center" :class="isDark ? 'text-white/90' : 'text-[#0A0A0A]'">{{ row.settlement }}</td>
                  <td class="px-8 py-6 text-[14px] text-center" :class="isDark ? 'text-white/90' : 'text-[#0A0A0A]'">{{ row.date }}</td>
    ...
    <div v-if="showPaymentModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-[999] flex items-center justify-center p-4">
      <div :class="isDark ? 'bg-[#111] border border-white/10' : 'bg-white'" class="rounded-[24px] shadow-2xl w-full max-w-[650px] overflow-hidden">
          ...
          <div class="space-y-6">
            <div class="grid grid-cols-2 gap-6">
              ...
              <div class="space-y-1.5 relative">
                <label class="text-[15px] font-normal" :class="isDark ? 'text-white/70' : 'text-[#1a1a1a]'">Select Partner *</label>
                <div @click="showPartnerList = !showPartnerList" 
                  ...
                  <span :class="[selectedPartner === 'Choose a partner' ? 'text-[#b0b7c1]' : (isDark ? 'text-white' : 'text-[#000]')]" class="text-[15px]">{{ selectedPartner }}</span>
