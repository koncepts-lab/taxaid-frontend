<template>
  <div class="min-h-screen w-full relative flex flex-col font-sans transition-colors duration-300" :class="isDark ? 'dark-mode-bg text-white' : 'bg-[#f3f4f6] text-[#1a1a1a]'">
    ...
    <!-- HEADER -->
    <AdminDashboardHeader :userName="admin?.role?.name ?? 'Review Manager'" :userId="'Welcome, ' + (admin?.full_name ?? '')" :showChangeProfile="false" :adminLogout="true" logoutTo="/ad-aqnz-pro-auth-78z46" />
...
function goBack() {
  navigateTo('/admin/review-team/manager/dashboard')
}
</script>
