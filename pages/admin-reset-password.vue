<template>
  <div class="min-h-screen bg-[#f4f6f5] flex items-center justify-center p-4">
    ...
    <div v-else-if="pageState === 'invalid'" class="bg-white rounded-2xl shadow-md w-[440px] max-w-full p-10 text-center">
      ...
      <NuxtLink to="/ad-aqnz-pro-auth-78z46" class="text-[#007C65] text-sm font-medium hover:underline">Back to Login</NuxtLink>
    </div>
    ...
    <div v-else-if="pageState === 'form'" class="bg-white rounded-2xl shadow-md w-[440px] max-w-full">
      <div class="px-8 pt-8 pb-2 text-center">
        ...
        <p class="text-sm text-gray-500 mt-1 mb-0">Hi <span class="font-medium text-gray-700">{{ tokenName }}</span>, choose a new password for your TaxAid admin account.</p>
      </div>
    ...
    <div v-else-if="pageState === 'success'" class="bg-white rounded-2xl shadow-md w-[440px] max-w-full p-10 text-center">
      ...
      <NuxtLink to="/ad-aqnz-pro-auth-78z46" class="inline-block bg-[#007C65] text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-[#006A56] transition-colors">Go to Login</NuxtLink>
    </div>
  </div>
</template>
...
// Only `ref` is the real token — `sid`/`v`/`trace` are decoys from the email link, never read.
const token = route.query.ref
...
onMounted(async () => {
  ...
  try {
    const res = await $fetch('/admin/reset-password/validate', {
      baseURL: config.public.apiBase,
      params: { token },
    })
    if (res.valid) {
      tokenName.value = res.name
